...
" 2>/dev/null | while IFS='|' read -r uuid filename created modified added type width height favorite hidden lat lon; do
    ...
    echo "Hidden: $hidden"
    if [ -n "$lat" ] && [ "$lat" != "" ] && [ "$lat" != "0" ] && [ "$lat" != "-180.0" ]; then
