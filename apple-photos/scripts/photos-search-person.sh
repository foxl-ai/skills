...
SELECT Z_PK FROM ZPERSON WHERE LOWER(ZDISPLAYNAME) LIKE LOWER('%$NAME%')
...
sqlite3 "$PHOTOS_DB" "
SELECT DISTINCT a.ZFILENAME, datetime(a.ZDATECREATED + 978307200, 'unixepoch', 'localtime'),
    CASE a.ZUNIFORMTYPEIDENTIFIER
        ...
        ELSE a.ZUNIFORMTYPEIDENTIFIER END, a.ZUUID
FROM ZASSET a JOIN ZDETECTEDFACE f ON f.ZASSETFORFACE = a.Z_PK
WHERE f.ZPERSONFORFACE IN ($PERSON_IDS) AND a.ZTRASHEDSTATE = 0 AND a.ZFILENAME IS NOT NULL
...
SELECT COUNT(DISTINCT a.Z_PK) FROM ZASSET a
JOIN ZDETECTEDFACE f ON f.ZASSETFORFACE = a.Z_PK
WHERE f.ZPERSONFORFACE IN ($PERSON_IDS) AND a.ZTRASHEDSTATE = 0;
" 2>/dev/null)
echo "---"
